...
- 用户动态与用户活动的地图弹窗重新开放「关联发帖」；新帖子可关联官方活动或另一条用户帖子，关联帖子在地图弹窗和详情页展示，并新增兼容旧数据的自关联数据库迁移。
- 用户动态和用户发布活动详情的地图入口改为地图定位，不再直接开启导航，并收起遮挡定位点的附近活动面板；有坐标但没有填写地点名时也能进入地图。用户内容详情现在展示已关联足迹；地图弹窗移除用户内容上无法真正保存的「关联发帖」，官方活动仍保留。
- 发现页活动卡片与今日推荐增加公开足迹数量及直达入口，活动详情展示关联足迹；活动和动态在首批快照耗尽后继续从服务器分页读取，足迹保留原有分页与失败重试。

## 2026-09-24

- 发现页足迹卡片完整展示最多 6 个已选心情标签：首个保留在卡片头部，其余标签在下方按宽度换行，标签文字保持单行。
- 修复窄屏足迹卡片的心情标签被昵称与照片挤压后换行的问题，昵称可截断，标签保持单行完整显示。
- 发现页重排为「逛活动 / 看动态·足迹」双内容入口：保留今日推荐横向轮播并将箭头和页点叠在海报上，活动列表改为双列瀑布流，动态与足迹合并为按时间排序的单列内容流，列表继续滚动加载；顶部沿用日历页紧凑标题栏与可收起搜索，恢复较宽的内容切换 tab。
- 发现页将每日个性化推荐与原精选横幅合并为可左右滑动的活动卡，保留推荐理由及「想去／不感兴趣」操作；活动海报加大并补足无图占位。活动和社区动态在接近列表底部时逐批展示更多内容，足迹继续请求后续分页并避免失败后自动重复重试。
- 发现页的动态与足迹模块标题改为不依赖筛选状态的“社区动态”和“大家的足迹”，同步更新日语与英语文案。

## 2026-09-23

- 天气详情不再固定限制为半屏高度，会优先利用当前视口的剩余空间完整展开；仅在矮屏或内容确实超出安全区域时启用纵向滚动。
- 地图天气面板改为两层浏览：默认保留七日概览，点击日期后以符合应用玻璃卡片风格的滑动渐显动画展开体感温度、湿度、最大风速、日出日落、可能降雨时段及三小时间隔预报；面向用户的内容不再暴露天气供应商缩写或技术来源。
- 东京天气改为优先读取日本气象厅官方 JSON，提供短期及七日预报、分时降水概率、预报可信度与天气概况，并在不可用时回退 Open-Meteo 的 JMA 模型；AI 导游同步获得完整可用预报范围，回答对应日期的出行问题时会结合天气、气温和降水概率。
- 社区模拟的东京天气改为复用地图现有的 Open-Meteo 七天真实预报，并把气温范围与降水概率带入人物决策和图片生成；补跑历史日期或天气服务不可用时继续使用原有的季节性稳定回退。

## 2026-09-21

- 地图中的官方活动改为按固定地理网格独立加载，并在浏览器中缓存后立即显示、后台刷新；用户动态与足迹仍保持实时请求及原有显示优先级，减少重复进入地图时等待官方活动的时间。
- 活动详情中的地址旁入口改为只在地图上定位活动地点，不再打开乘换案内；顶部路线按钮继续用于规划路线。
- 修复乘换案内最小化后仍露出并可点击全局底部菜单、切换任意页面时因清理已卸载地图而报错的问题。
- 中文活动分类将“节庆”恢复为“祭典”，地图、发现、日历、发布和详情继续共用同一语言包标签。
- 路线查询的站名联想保留并优先展示完整精确匹配，修复输入完整站名后候选消失的问题；出发站与目的地标签加宽并保持单行，避免日语界面折行；用户允许定位后，空白出发地默认使用当前位置且不覆盖手动输入。
- 发现页与日历搜索改为查询完整活动库并按标题相关度排序，日历会自动跳转到命中日期；默认浏览范围扩展至东京周边的群马、长野、栃木、茨城、静冈、山梨等区域，并优先加载仍可参与的活动。
- “活动 > 想去”状态提示完成中日英本地化；日语足迹按钮改为「足跡を残す」，并调整双按钮宽度分配与长文案换行，避免英语和日语在窄卡片中贴边溢出。
- 每日活动抓取在写库前自动应用已提交的 Prisma 迁移，修复无坐标活动写入时因线上 `Event.lat/lng` 仍为非空字段而触发的 `P2011`。

## 2026-09-20

- 虚拟人物凌晨一次性规划当天的发帖、足迹、评论与互动，并直接使用未来 `createdAt` 作为发布时间；列表、详情、互动数量和消息通知只读取已到发布时间的数据，无需轮询或新增字段。
- 每日活动拉取保留无地址或地址解析失败的官方活动：可在发现页查看，但不进入地图数据与路线操作。

## 2026-09-17

- 修复个人生活动态详情在窄屏出现页面级横向滚动条的问题，并放宽私信图标的点击区与图形留白。
- 用户动态详情顶部恢复为单行排版，私信改用对话图标、关注状态保留文字，避免日语与英语按钮撑乱布局或增加顶部高度。

## 2026-09-16

- 用户动态详情顶部的私信与关注操作改为用户名下方的自适应单行按钮，避免日语与英语长文案挤乱顶部布局。

## 2026-09-13

- 车站弹窗适当加宽并让导游按钮保持单行，修复英语与日语界面按钮折行；地图地标简介统一接入中日英公共语言包。
- 活动分类、WMO 天气状态、餐饮菜系与地标类型完成中日英数据标签适配；地图类型映射集中到公共本地化配置，日历、筛选、发布编辑、详情、个人页和地图弹窗不再直接展示中文元数据标签。
- 40 个心情标签完成中日英适配，并在选择器、地图足迹、个人页和探索列表共用公共语言键；地图 AI 意图操作卡及活动持续天数同步改为三语参数化文案。
- 补齐日历日期、发布与足迹图片区、筛选分享、心情选择计数、活动评论操作、换乘末段步行、测试账号状态、IP 伙伴名称与页面加载提示的中日英适配；个人伙伴名称在 AI 导游中同步跟随界面语言。
- AI 导游完成中日英界面适配：欢迎与话题入口、快捷问题、思考进度、路线卡片、后续建议、聊天记录提示、输入操作、活动打开状态及清空确认统一使用公共语言包。
- 线路详情面板完成中日英适配：运行状态、方向切换、班次与逐站时刻、实时列车及延误、站点状态和无数据提示统一使用公共语言包。
- 个人页资料菜单适当加宽，编辑资料与登出项强制单行显示，避免日语界面换行。
- 地图车站、地标与餐饮原生弹窗完成中日英适配：线路摘要、导航、导游入口、相关发帖与足迹提示、特色菜品以及发布结果统一使用公共语言包。
- 地图主界面与活动原生弹窗完成首批三语迁移：底部地图工具栏、发布类型、到达与足迹成功提示、活动卡片详情/发帖/足迹页签、导航收藏及相关内容状态使用公共语言包；地图足迹日期按当前语言格式化。
- 探索列表完成中日英适配：顶部导航、搜索与日期筛选、官方精选、热门与全部活动、用户发帖与足迹、评论回复、加载及空状态、管理删除确认统一使用公共语言包；相对时间按当前语言显示，并新增公共活动分类翻译键映射。
- 个人页主体完成中日英适配：足迹、发帖、管理、活动集合、互动消息、空状态、编辑删除、路线与到访操作统一使用公共语言包；月份分组、活跃天统计和活动时间按当前语言格式化。
- 推荐首页与地图附近推荐完成中日英适配：每日精选、推荐理由与注意事项、活动来源与分类、地图空状态、收藏入口和场景化 AI 规划问题均使用公共语言包，日期按当前语言格式化。
- 地图发布入口、底图样式切换、活动来源徽标与筛选、足迹评论线程完成中日英适配，评论日期跟随当前语言显示。
- 完成第三批三语界面迁移：个人资料编辑与关注列表、私信会话、动态及足迹编辑、日期范围与时间选择器、图片排序提示统一使用公共语言包；日期和私信时间按当前语言格式化。
- 完成第二批三语界面迁移：活动详情、评论与互动、确认弹窗、分享复制、图片预览、安装提示、全局加载页、AI 导游入口和底部表单容器统一从公共语言包取文案；活动日期时间按当前语言格式化。
- 扩充中、日、英公共语言包并完成第一批全站界面迁移：登录注册、地图筛选、天气、日历、发帖、打卡和换乘路线不再硬编码中文；动态数量采用参数化文案，星期与时间按当前语言格式化。
- 调整活动与用户帖子正文操作顺序：翻译按钮紧接正文行尾显示，“展开更多/收起更多”移到其后，不再让展开控制隔开正文和翻译入口。
- 将中、日、英界面文案从 React Provider 拆分到 `src/i18n/locales/` 公共语言包，并集中维护支持语言、默认语言及持久化键，供页面和服务端接口复用。
- 个人动态详情顶部的返回、点赞、收藏、分享及折叠入口移除按钮阴影，不再继承官方活动悬浮工具栏的视觉样式。
- 未匹配中文、日语或英语的系统语言现在默认使用英语；活动详情与用户帖子正文末尾新增轻量翻译按钮，通过服务端 LLM 按当前界面语言翻译，并支持原文/译文切换与失败重试。
- 新增中文、日语、英语三语本地化基础层：首次访问按系统语言显示；个人资料设置可即时切换并在当前设备持久化，底部主导航同步完成三语适配。
- 第一行第二款开屏 Logo 直接补入“云迹 / CLOUD FOOTPRINT”两行字标，并以新文件名同步网页开屏、PWA 清单、Apple 图标及安装提示，避免桌面端原生启动阶段缺字或继续命中旧缓存。
- 中文字标改用更接近设计稿的纤细现代衬线体；网页开屏保留“发现 · 出发 · 留下足迹”副标题，并移除重复的可见品牌标题。

## 2026-09-11

- 未登录用户在活动详情中点赞、收藏或报名时，改为显示无装饰图标、避开底部菜单的紧凑居中登录提示，并可直接前往个人页登录。
- 官方活动详情页使用两侧留白的圆角悬浮吸顶栏，四个操作按钮圆圈尺寸统一；离开页面顶部后显示半透明模糊背景，并保留按钮圆圈 20% 的透明度。
...
- AI 导游的流式正常结果、截断正文和纯文本降级统一补足至少 3 个相关追问；模型未返回或补全追问失败时使用本地导游问题兜底，不再因 DeepSeek 降级丢失二次追问。
- AI 导游以 `finish_reason` / `[DONE]` 校验 DeepSeek 流是否完整，并忽略空事件和未完成尾帧。所有思考任务统一按“原配置 → 关闭思考并保留 JSON 模式 → 关闭思考并取消强制 JSON”的顺序降级；普通导游输出上限提高到 24000 token，高思考路线与长期人物决策提高到 32000 token，避免推理耗尽额度后没有最终正文。
- DeepSeek 默认模型、运行时回退、本地配置示例与模拟工作流统一切换为官方 V4.1 Flash 模型名 `deepseek-flash`。

### AI 任务分级与导游体验
- 新增按任务 key 集中维护的 LLM 配置，统一控制思考开关、思考强度和输出额度；导游路线与人物长期决策使用更高思考等级，抽取、改写等确定性任务关闭或降低思考。
- AI 导游提示词改为先回应真实诉求、使用自然亲近的表达，并在结尾结合时间、预算、出发地或路线顺势引导下一步。
- 导游等待期间展示“理解问题、比较活动、核对时空、整理建议”等进度反馈；服务端增加 90 秒超时、请求 ID、阶段和耗时日志，便于定位模型拥堵与失败。
- 附近活动候选清单改为仅通过后台上下文发送，用户消息只显示所选操作；活动详情加载改为面板中央遮罩反馈，慢请求期间也能清楚看到进度和取消入口。
- 导游普通回答额度提高至 10000 token、模型超时延长至 180 秒，并集中在任务配置中维护；服务端运行上限同步放宽，已收到正文但尾部 JSON 元数据被截断时保留正文，不再误报整次中断。

### 虚拟人物连续性优化
- 每日足迹与社区内容增加低成本质量门：检查人物禁用表达、助手腔、近期重复、活动状态误写及明显现实约束冲突；失败时使用当前模型定向重写一次，仍失败则阻止不合格内容落库。
- 每日与社交决策共用分层记忆上下文，同时读取近期经历、重大记忆、关系/目标记忆和生活摘要，避免长期主线被日常琐事覆盖。
- 决策提示明确区分私下经历与公开表达，重大旧事不得伪装成当天事件；目标只能渐进推进、受阻或搁置，避免无铺垫跳变。
- 人物状态开始消费新增的活动足迹、想去、收藏和报名行为；游标防止重复结算，实际参加与报名会以受控幅度影响精力、社交余量、预算和安排压力。
- 角色状态增加兼容的结构化目标和短期现实状态；每日可有限更新一个既有目标，并让精力、工作负担、社交电量和预算压力约束实际行为。

### DeepSeek 模型切换
- 默认、本地与示例配置从已弃用的 `deepseek-chat` 别名切换为官方当前模型名 `deepseek-flash`，并同步 README。

## 2026-09-09

### 修复锚点周边展开
- 修正天气退散联动条件，避免“锚点周边”按钮点击后因自身展开状态被错误移出视野。

### 天气展开时地图控件退散
- 天气预报展开后，筛选、地图控件、底部工具栏和“锚点周边”卡片统一退散，天气内容保持完整可见。

### 地图天气与足迹路线联动
- 天气预报展开时，底部工具栏同步向下退散，避免遮挡天气卡片并释放可视空间。
- 开启“足迹路线”时自动勾选“只看我的”，路线只展示当前账号的足迹；关闭路线不会擅自取消用户已选的筛选。

### 地图“只看我的”筛选修复
- “只看我的”现在按当前登录账号的作者 ID 过滤活动与动态，并同步只保留当前账号的足迹；不再把所有用户及虚拟人物内容误认为“我的”。

### 部署构建 Sharp 依赖修复
- 将应用直接使用的 Sharp 与 Next.js 16.2.7 自带版本统一为 0.34.5，避免服务端构建同时打包两套原生模块后遗漏对应的 Linux libvips 动态库。

### 想去卡片按钮排版
- “想去”中的双列活动卡片操作按钮统一保持单行，已结束活动的“去过，留足迹”缩短为“记足迹”，避免窄屏按钮折行并撑乱卡片底部。

### 地图天气点击修复
- 天气按钮在地图工具栏退避容器中恢复独立点击能力，点击不再穿透到地图或探索锚点；透明区域仍可正常拖动地图。

### 探索详情返回
- 活动详情打开时再次点击底部当前“探索”，现在会关闭详情与图片预览并回到保留原滚动位置的探索列表；不再因为重复导航到同一路由而表现为无响应。

### 官方活动高清图片
- 官方活动采集遇到 WalkerPlus 小尺寸缩略图时，改为优先提取并验证活动官网的高清社交分享图，只有宽度达到 640px 才写入。
- 新增可预览后执行的存量图片刷新脚本，默认优先当前与未来活动并以受控并发检查，只替换已确认的 WalkerPlus 缩略图；抓取失败或候选图仍过小则保留原图。
- 活动详情遇到尺寸明显不足的源图时不再强行铺满放大，改为清晰主体配柔化背景，减少像素化观感。
- 官方活动主图解除渐变与标题覆盖层的点击拦截，整张图片均可打开全屏预览，并增加明确的“查看大图”入口，方便阅读海报内容。

## 2026-09-08

### 消息分类与准确已读
- 点击个人页“消息”或切换到“互动”分类不再清空提醒；私信仅在打开具体会话后已读，互动仅在成功打开对应足迹或活动后逐条已读。
- 互动已读状态改为按账号保存的逐条 ID 集合并限制记录数量，旧的一键清空时间戳不再继续吞掉未实际查看的提醒。
- 消息页强化“私信 / 互动”双分类，分别展示用途说明、独立未读数和互动条目的未读标记。

## 2026-09-07

### 跨页面状态恢复与真机回归
- 探索、日历与地图筛选改为版本化的标签页会话状态，支持页面切换及当前标签刷新后恢复；Set 类型筛选可正确还原，大型活动/足迹数据仍只留在内存，避免会话存储膨胀和账号数据串用。
- 探索/日历的嵌套滚动位置与地图中心、缩放、旋转、俯仰及探索锚点加入会话恢复；地图详情返回后可回到原视野，其他入口关闭详情仍停留原页。
- 新增真机设备矩阵、核心用例、证据要求与通过标准，并提供 `npm run test:regression` 自动检查推荐排序和会话序列化。

### 附近活动推荐排序与解释
- “附近活动”不再先截取最近 8 个再做场景排序，改为统一综合活动时效、距离、明确场景意图、今日精选、人气及信息完整度后选出结果。
- 活动卡片新增最多两条“为什么推荐”标签，例如“正在进行”“今天可去”“即将结束”“适合拍照”和实际距离；相同输入保持稳定排序。
- 新增可重复运行的推荐排序断言脚本，覆盖默认排序、场景意图、解释文案和确定性。

### 地图首屏与弹窗层级收口
- 地图弹窗、附近活动、发布菜单、发布表单、路线与线路面板统一为互斥主界面；打开新的主界面会收起地图临时菜单与已有推荐，避免多层内容同时争抢空间。
- 顶部筛选、天气、地图控件及底部工具栏统一跟随主界面向四周退避；发布菜单打开时底部工具栏从菜单下层退出，关闭后再恢复。
- 美食、“更多”和发布菜单补齐互斥切换，地图表单或路线面板开启时忽略底层点位弹窗，减少误触穿透。

### 地图发布入口 IP 图标
- 保留云足·晴空原有的双色描边脚印，为云足·樱梦、路灵·远行、路灵·花语分别新增樱花爪印、指南针定位、花瓣指南针三枚同风格 SVG 发布徽记。
- 地图工具栏“发帖”按钮根据当前 IP 即时切换对应徽记；“不使用 IP”时使用同一描边语言的通用加号徽记。

### 图片排序与预览
- 发布和编辑动态、活动、足迹时，可直接拖动整张图片实时插队排序；换位加入平滑位移动画，不再显示单独的拖动图标。
- 轻点图片可打开全屏大图预览，多图支持前后切换、键盘方向键与 `Esc` 关闭。
- 拖动插队后增加位移滞回锁，手指停留不再因布局换位动画反复命中相邻图片而无限抖动。

### 发布表单紧凑化
- 动态、活动和足迹发布表单统一缩小标题区、字段高度、区块间距、分类按钮与圆角；图片改为四列排序网格，次要的地点、标签、报名和活动关联按需展开。
- 取消与发布操作移入固定底部栏，滚动填写时保持可见，并为表单正文释放更多连续空间。

### 发布与编辑图片排序
- 发布动态、活动和足迹，以及编辑动态、活动和足迹时，图片支持整图拖动和键盘方向键换位；第一张图片会明确标记为封面，并按调整后的顺序上传或保存。

### 官方活动详情操作区
- 缩小“想去”“路线”“AI 导游”三个主操作的高度、字号、图标、间距与圆角，降低操作区的视觉占用。

### 日历页导航与活动卡片改版
- 收紧日历页整体纵向节奏，降低年月、日期与日期数字字号，并缩小日历控件、活动卡片间距及大面积圆角，让页面更紧凑利落。
- 月份切换箭头归入年月标题两侧，“今天”作为独立操作放在右侧，避免箭头被误解为前后日；日期活动提示由三个圆点收敛为单条密度标记。
- 日期结果区去除重复时间轴和“展期”标签，改为图片、状态、标题、地点与结束日期分层的横向长条卡片。
- “当天开始 / 展期中”简化为“新开始 / 进行中”，进行中活动优先按结束时间排列并突出即将结束的项目。

### 虚拟人物社交推演稳定性
- 社交提示词截断改为按 Unicode 字符处理，避免在 emoji 中间截断后触发 DeepSeek JSON 解析错误。
- 单个虚拟人物的社交生成失败时记录错误并继续处理其他人物，不再导致当天全部动态和评论归零。

### 足迹发布加载反馈位置
- 足迹上传照片和保存期间的加载反馈改为表单弹层中央遮罩，与动态、活动发布保持一致，不再出现在长表单最底部。

### 地图弹窗聚焦体验
- 活动、聚合、景点、美食、车站和足迹共用同一个地图弹窗实例，重合点位点击时只保留最后触发的一张卡片。
- 地图弹窗打开时，筛选、天气、定位缩放、附近活动和底部地图工具会向屏幕四周退场；主导航固定在地图工具之上，底部工具会从它的下层退出，关闭卡片后自动恢复。
- 暂时移除会遮挡日期筛选且与探索锚点重复的“搜索此区域”按钮，地图拖动和缩放不再主动弹出额外推荐入口。

## 2026-09-06

### 动态与活动发布反馈位置
- 发布中的加载反馈改为表单弹层中央遮罩，不再跟随长表单出现在最底部，并在请求期间阻止重复操作。
- 发布完成提示移到屏幕中央偏上的高层级区域，避免被“附近活动”抽屉遮挡。
- 发布活动的时间范围改为与编辑页一致的“开始时间 / 结束时间”日期时间选择器，可同时选择日期和具体时刻。
...
- 按用户提供的月相系列替换开屏符号，基于平均朔望月按日期选择六种月相；六张原稿裁图与页面统一为纯白底，浏览器原生启动阶段使用满月静态图标兜底。
### 今日推荐每日轮换与即时补位
- “今日推荐”加入按东京日期生成的稳定轮换排序；已点“想去”或“不感兴趣”的活动会退出推荐位并立即由新候选补上，不再因想去加权而长期固定显示。候选池从 18 条扩大到 36 条，保留分类多样性和用户偏好信号。
### 生活动态顶部操作去底色
- 个人生活动态详情顶部的点赞、收藏、分享操作展开后改为透明底，移除白色圆底、阴影与模糊层，保留图标状态和点击范围。
### 动态私信输入栏恢复显示
- 提升个人生活动态详情的全屏层级，避免从动态进入私信后底部输入栏被全局导航遮住。
### 个人页关注列表避免菜单遮挡
- 提升关注与粉丝列表弹层的显示层级，使列表底部和操作按钮完整显示在全局菜单栏上方。
### 桌面开屏固定使用 02 标志
- 开屏取消按日期轮换，固定使用用户选定的第一行第二款月牙、云朵与爪印标志；主素材提升到 1024px 并绕过图片二次压缩，保留“云迹东京”和副标题文字，同时同步桌面/PWA 图标。
