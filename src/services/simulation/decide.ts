...
- photoDesc 必须写清楚镜头是谁拍的。独自行动时大约 70% 是“我用手机拍到的画面”（环境、餐桌、票根、书、街景、舞台、车窗、手边物品），约 20% 可以是合理的自拍/镜中倒影/定时器/三脚架/把手机放在桌上或地上延时自拍，约 10% 才是朋友帮拍或合照。
- 主角作为拍摄者时，画面里最多出现一只手；不要写成两只手同时清晰入镜，除非明确是自拍、镜子、定时器、三脚架、手机放置拍摄或朋友拍摄。
- 主角可以出镜，但必须说明合理拍摄方式：自拍、朋友同行、合照、朋友帮拍、镜中倒影、橱窗倒影、背影、侧脸、定时器、三脚架、手机放桌上/地上延时拍。
...
const JSON_INSTRUCTION = `只输出一个 JSON 对象，不要解释或代码围栏：
{
  ...
  // 或发足迹：
  "post": {"note": "50~150字第一人称足迹", "rating": 4, "spotIndex": 0, "photo": true, "photoDesc": "我用手机随手拍：窗边的手冲咖啡，目黑川的绿意在杯子后面虚化，最多一只手扶着杯子边缘"},  // rating 1-5 或 null；**大部分有场景/画面感的足迹都配图(约2/3)**；photoDesc 要标明拍摄方式。多数是主观镜头；少量可写自拍/镜中倒影/三脚架/手机放桌上或地上延时自拍/朋友帮拍/合照
