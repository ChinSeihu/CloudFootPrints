...
`sim-init.ts` 初始化 `CharacterState` 时会把 V2 目标转换为带进度、动机、阻碍和下一步的结构化目标，同时初始化精力、工作负担、社交电量与预算压力。旧的 `goals` 字符串数组继续保留作为兼容来源。

每日决策最多更新一个既有目标，进度与动机变化会被代码限幅；进度不足 90 时不能直接标记完成。短期现实状态也采用 0–100 范围和单日限幅，避免一次生成造成不可信的剧烈变化。

社区发帖、评论和回复也会读取这些状态。工作负担高或社交电量低时，角色更少主动互动，文字会体现当下的现实余量。

人物每日推演还会读取上次游标之后新增的活动行为：关联活动的足迹视为实际参加，`WANT` 视为意向，`SIGNUP` 视为已形成安排。实际参加会消耗精力、社交电量并产生有限预算压力，报名会增加近期工作量；收藏只作为兴趣证据。行为只结算一次，并作为目标推进的事实依据交给决策模型。

每日足迹和社区内容在写入前通过同一套人物质量门。代码会检查写作 DNA 禁用词、助手式表达、近期文本重复、把想去/报名误写成已经参加，以及高预算压力或极低精力下的明显矛盾。首次不合格会携带具体原因由当前模型重写一次；第二稿仍不合格时不发布，私下记忆存在事实错误时也不会写入人物状态。正常内容不增加额外模型调用。
...
- 第一人称主角拍摄时，画面里通常最多出现一只手；除非明确自拍、镜子、定时器、三脚架、手机放在桌上/地上延时拍或朋友拍摄，否则两只手同时清晰入镜会显得不合理。
