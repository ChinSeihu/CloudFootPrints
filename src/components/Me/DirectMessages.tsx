...
function chatTime(value: string, locale: string) {
  const date = new Date(value);
  const today = new Date();
  if (date.toDateString() === today.toDateString()) return date.toLocaleTimeString(locale, { hour: "2-digit", minute: "2-digit" });
...
export function DirectMessages({
  ...
  async function loadConversation(conversation: { id: string; other: DirectMessageUserDTO }, quiet = false) {
    ...
    try {
      ...
      if (!quiet) setError(t("message.loadFailed"));
    } finally {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (res.ok && data.conversationId && data.other) {
        ...
        setError(t("message.startFailed"));
      }
      setLoading(false);
    })();
  ...
  async function send() {
    ...
    try {
      ...
      if (!res.ok || !data.message) throw new Error(data.error || t("message.sendFailed"));
      ...
      setDraft(text);
      setError(sendError instanceof Error ? sendError.message : t("message.sendFailed"));
    } finally {
      setSending(false);
    }
  }
    ...
    return (
      <section className="fixed inset-0 z-[90] flex flex-col bg-neutral-50">
        <header className="sticky top-0 z-10 flex items-center gap-3 border-b border-neutral-100 bg-white/95 px-3 py-2.5 backdrop-blur">
          <button type="button" onClick={() => { if (onClose) onClose(); else { setActive(null); setMessages([]); void refreshConversations(); } }} aria-label={t(onClose ? "message.close" : "message.backToList")} className="grid size-8 shrink-0 place-items-center text-neutral-600">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-[11px] text-neutral-400">{active.other.status || active.other.signature || t("message.defaultStatus")}</p>
          </div>
        </header>

        <div className="flex min-h-0 flex-1 flex-col overflow-y-auto bg-neutral-50/70 px-3 py-4">
          {loading && <LoadingFeedback compact scene="message" text={t("message.loading")} />}
          {!loading && messages.length === 0 && <p className="m-auto text-center text-xs text-neutral-400">{t("message.sayHello")}</p>}
          <div className="mt-auto space-y-3">
              ...
              return (
                <div key={message.id} className={`flex items-end gap-2 ${mine ? "justify-end" : "justify-start"}`}>
                  ...
                  <div className={`max-w-[78%] ${mine ? "text-right" : "text-left"}`}>
                    ...
                    <p className="mt-1 px-1 text-[9px] text-neutral-300">{chatTime(message.createdAt, locale)}</p>
                  </div>
                </div>
              );
            })}
            {sending && active.other.virtual && (
              <div className="flex items-center gap-2 text-[11px] text-neutral-400">
                ...
                <span className="rounded-lg bg-white px-3 py-2 shadow-sm ring-1 ring-black/5">{t("message.typing")}</span>
              </div>
            )}
            <div ref={bottomRef} />
          </div>
        ...
        <div className="border-t border-neutral-100 bg-white px-3 py-3">
          ...
          <div className="flex items-end gap-2">
            <textarea value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void send(); } }} rows={1} maxLength={1000} placeholder={t("message.placeholder")} className="max-h-28 min-h-10 min-w-0 flex-1 resize-none rounded-lg bg-neutral-100 px-3 py-2.5 text-[13px] outline-none focus:ring-2 focus:ring-blue-100" />
            <button type="button" onClick={() => void send()} disabled={!draft.trim() || sending} className="h-10 shrink-0 rounded-lg bg-blue-600 px-4 text-xs font-bold text-white disabled:opacity-40">{t("message.send")}</button>
          </div>
        </div>
      </section>
    );
    ...
    return (
      <section className="fixed inset-0 z-[90] flex flex-col bg-neutral-50">
        <header className="flex items-center gap-3 border-b border-neutral-100 bg-white px-3 py-2.5">
          <button type="button" onClick={onClose} aria-label={t("message.close")} className="grid size-8 shrink-0 place-items-center text-neutral-600">
            <IconChevronLeft className="size-5" />
          </button>
          <h3 className="text-sm font-bold text-neutral-950">{t("message.title")}</h3>
        </header>
        <div className="grid flex-1 place-items-center px-6 text-center text-xs text-neutral-400">
          <p>{error || t("message.opening")}</p>
        </div>
      </section>
    );
  }
    ...
    <section>
      ...
      {conversations.length === 0 ? (
        <div className="py-12 text-center">
          <p className="text-sm font-semibold text-neutral-700">{t("message.empty")}</p>
          <p className="mt-1 text-xs text-neutral-400">{t("message.emptyHint")}</p>
        </div>
      ) : (
              ...
              <button type="button" onClick={() => { const next = { id: conversation.id, other: conversation.other }; setActive(next); void loadConversation(next); }} className="flex w-full items-center gap-3 py-3 text-left">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <time className="ml-auto shrink-0 text-[10px] text-neutral-300">{chatTime(conversation.lastMessageAt, locale)}</time>
                  </div>
                  <div className="mt-1 flex items-center gap-2">
                    <p className={`truncate text-xs ${conversation.unreadCount ? "font-semibold text-neutral-700" : "text-neutral-400"}`}>{conversation.lastMessage?.text || t("message.newConversation")}</p>
