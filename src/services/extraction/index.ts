...
import { ingestEvents, prefilterEventsForIngest, type IngestStats } from "./ingest";
...
function emptyStats(): IngestStats {
  return { considered: 0, geocodeFailed: 0, duplicates: 0, inserted: 0, updated: 0 };
}
...
// 跑单个源的完整管线：采集 → (LLM 抽取 | 直接用结构化) → 预去重/补旧时间 → LLM 增强 → 地理编码+入库。
export async function runSource(source: Source): Promise<IngestStats> {
  ...
  for (const doc of docs) {
    ...
    const prefiltered = await prefilterEventsForIngest(events, sourceMeta, doc.text ?? null);
    total = addStats(total, prefiltered.stats);
    events = prefiltered.events;
    if (events.length === 0) continue;
    ...
    const stats = await ingestEvents(
      ...
      sourceMeta,
      doc.text ?? null,
      { countConsidered: false },
    );
    total = addStats(total, stats);
  }
  console.log(
    `  ✓ ${source.name}：考察 ${total.considered}，入库 ${total.inserted}，` +
      `更新 ${total.updated}，去重 ${total.duplicates}，地理编码失败 ${total.geocodeFailed}`,
  );
  return total;
}
