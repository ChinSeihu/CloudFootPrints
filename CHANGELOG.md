...
- **推荐页顶部重做**（更贴近小红书）：去掉「中午好…」问候行；一级菜单「活动 / 发现」居中、选中字号加大加粗 + 蓝色短下划线；**搜索图标移到右上角**（无边框纯图标）；二级分类横滑标签与漏斗同处一行，漏斗也改为无边框图标。整体从 3 行压到 2 行，更清爽。
- **日历页筛选精简**：去掉独占一行的「全部来源/官方/个人」来源筛选，以及清单项里的来源徽标（与推荐页一致），筛选区只留分类横滑标签，不再拥挤。
