...
export function InstallPrompt() {
  ...
  return (
    <aside role="dialog" aria-label={t("install.dialog")} className="fixed bottom-[calc(5.25rem+env(safe-area-inset-bottom))] left-3 right-3 z-[45] mx-auto max-w-sm overflow-hidden rounded-2xl border border-white/80 bg-white/95 p-3 shadow-[0_18px_52px_rgba(15,23,42,0.24)] backdrop-blur-xl">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm font-black text-slate-900">{t("install.title")}</p>
          <p className="mt-0.5 text-xs leading-5 text-slate-500">{t("install.description")}</p>
        </div>
        <button type="button" onClick={() => { rememberDismissal(); setVisible(false); }} aria-label={t("install.close")} className="grid h-8 w-8 shrink-0 place-items-center rounded-full text-lg text-slate-400 hover:bg-slate-100">×</button>
      </div>
      {showHelp && <p role="status" className="mt-2 rounded-xl bg-sky-50 px-3 py-2 text-xs leading-5 text-sky-800">{isApple ? t("install.appleHelp") : t("install.browserHelp")}</p>}
      <div className="mt-2 flex justify-end gap-2">
        <button type="button" onClick={() => { rememberDismissal(); setVisible(false); }} className="min-h-10 rounded-full px-3 text-xs font-semibold text-slate-500">{t("install.later")}</button>
        <button type="button" onClick={() => void install()} className="min-h-10 rounded-full bg-gradient-to-r from-violet-600 to-sky-500 px-4 text-xs font-bold text-white shadow-sm">{installEvent ? t("install.action") : t("install.help")}</button>
      </div>
    </aside>
  );
}
