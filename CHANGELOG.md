...
地图放大到一定缩放级别后，在图标下方显示一句 ≤10 字的简介标签（超出截断加省略号，用 MapLibre 表达式渲染时截断，不改数据）：

- **活动**：原本单点无文字，现在 `event-glyph` 层加上活动标题标签（zoom 14→14.6 淡入）。
- **美食 / 景点**：原本显示会换行的全名，改为单行 ≤10 字（`shortLabelExpr`，`text-max-width` 10）。
