...
- Detail drawer and interactions: `src/components/Recommend/EventDetail.tsx` — loads related public footprints through the existing related API.
...
- Weather: `src/services/weather.ts` normalizes official JMA JSON, supplements hourly detail through the Open-Meteo JMA model, and builds guide context; `src/app/api/weather/route.ts`, `src/components/Map/WeatherPanel.tsx` (seven-day overview plus selected-day detail), and `WeatherAnimation.tsx` consume the shared forecast.
...
- `PageLoading.tsx` — delayed route-loading feedback plus the fixed high-resolution 02 initial brand splash with HTML-rendered wording; calendar/discovery use continuous IP scenes.
...
- `TodayPicks.tsx` — explainable daily top-three recommendations that rotate by Tokyo date and replace wanted or dismissed cards; `src/lib/dailyPicks.ts` owns deterministic selection.
