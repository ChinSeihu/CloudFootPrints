...
export function CheckInDialog({ lat, lng, eventId, targetTitle, nearbyEvents = [], onCancel, onSubmit, onSnapChange }: Props) {
  ...
  const [selectedEvent, setSelectedEvent] = useState<CheckInEventOption | null>(
    eventId ? { id: eventId, title: targetTitle ?? t("checkin.selectedEvent") } : null,
  );
  ...
  return (
    <BottomSheet
      ...
      hint={selectedEvent ? t("publish.linkedTo", { title: selectedEvent.title }) : t("checkin.hint")}
      onClose={onCancel}
    ...
    >
      <div className="mb-3.5 overflow-hidden rounded-xl border border-neutral-200 bg-white">
        <button type="button" onClick={() => setAssociationOpen((open) => !open)} aria-expanded={associationOpen} className="flex w-full items-center justify-between gap-3 px-3 py-2.5 text-left">
          <span className="min-w-0">
            <span className="block text-xs font-semibold text-neutral-800">{t("checkin.association")}</span>
            <span className="mt-0.5 block truncate text-[10px] text-neutral-400">{selectedEvent?.title ?? t("checkin.mapOnly")}</span>
          </span>
        ...
        {associationOpen && (
          <div className="space-y-1.5 border-t border-neutral-100 p-2.5">
            ...
            <div className="border-t border-neutral-100 pt-2">
              ...
              <p className="mt-1 text-[10px] text-neutral-400">{searchingEvents ? t("checkin.searching") : eventQuery.trim().length === 1 ? t("checkin.minSearch") : t("checkin.searchHint")}</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="mb-3.5">
        ...
        {canUpload ? (
          ...
          <p className="rounded-lg bg-amber-50 px-3 py-2 text-[11px] text-amber-600">{t("checkin.noUpload")}</p>
        )}
      </div>
      ...
      <div className="mb-2 rounded-xl border border-neutral-200 bg-white px-3 py-2.5">
        <button type="button" onClick={() => setIsPublic((value) => !value)} className="flex w-full items-center justify-between gap-3 text-left">
          <span>
            <span className="block text-xs font-semibold text-neutral-900">{isPublic ? t("checkin.public") : t("checkin.private")}</span>
            <span className="mt-0.5 block text-[10px] leading-relaxed text-neutral-500">{isPublic ? t("checkin.publicHint") : t("checkin.privateHint")}</span>
          </span>
