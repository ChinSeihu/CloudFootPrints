...
- **Phase 4（已实现 · 管线）**：人物配图管线 `image.ts`。`decide` 标注 `photo`/`photoDesc`；`ImageProvider` 接口 + `getImageProvider()`（`IMAGE_PROVIDER` env：`none` 默认 / `agnes`）；`buildPrompt` 以 `personas.appearance`(外观基准 `public/person.png`) + `photoSkill` 视角（casual/hobby 主观手机镜头、pro 客观）+ 画面描述 + 季节天气拼 prompt；生成图 `persistToCloudinary`（CORS 持久）。引擎在足迹发布且 `photo` 为真时生成→上传→回填。**provider 失败一律不出图、绝不打断 sim**。**已接 Agnes（当前选用，OpenAI images 兼容，端到端实测出图正常）**：`IMAGE_PROVIDER=agnes` + `AGNES_API_URL`(base，如 `https://apihub.agnes-ai.com/v1`) + `AGNES_API_KEY` + `AGNES_MODEL`(默认 `agnes-image-2.1-flash`)；`POST <base>/images/generations` Bearer，取 `data[0].url`，上传 Cloudinary。备选 **Gemini 2.5 Flash Image**（`IMAGE_PROVIDER=gemini` + `GEMINI_API_KEY` + 可选 `GEMINI_IMAGE_MODEL`；鉴权 `x-goog-api-key` 头，已实测确认；图片在 `candidates[0].content.parts[].inlineData` base64 → data URI → Cloudinary）。备选 Agnes 同接口。当前所给 Gemini key **账户额度耗尽(429)**，充值后即可出图。**人脸一致性（已实现）**：`scripts/crop-refs.ts`(sharp) 把 person.png 裁成 12 张单人参考图 `public/refs/NN.png`；出图按 `refIndex` 载入，作 Agnes `extra_body.image`(img2img) 传入，把人物放进新场景并锁脸（实测有效）。取舍：img2img 下穿搭倾向参考图那套（脸硬锁、穿搭软变）。**清空重灌**：`scripts/sim-reset.ts`(--yes 才删) 只清 demo 内容+模拟状态、重置 status/signature、保留账号；重灌 = reset→seed-demo→sim-init→sim-run 回填。
- **配图 prompt = LLM 撰写 + 规则附加（已实现）**：生成前先用 LLM(`scenePromptLLM`，provider 感知)写专业详细英文场景 prompt，再 `composePrompt` 附加 `buildRules`(视角/写实/表情/外观一致/无水印)。LLM 失败回退 photoDesc+规则。Provider 收最终 prompt 字符串。
