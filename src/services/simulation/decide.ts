...
- photoDesc 必须写清楚镜头是谁拍的。第一人称独自行动时，默认是“我用手机拍到的画面”，优先拍环境、餐桌、票根、书、街景、舞台、车窗、手边物品，不要默认生成“别人拍我”的第三人称肖像。
- 主角作为拍摄者时，画面里最多出现一只手；不要写成两只手同时清晰入镜，除非明确是自拍、镜子、定时器、三脚架或朋友拍摄。
- 只有文案明确自拍、朋友同行、合照、朋友帮拍、镜中倒影、背影、被拍瞬间时，photoDesc 才能让发帖人本人明显出镜。
...
const JSON_INSTRUCTION = `只输出一个 JSON 对象，不要解释或代码围栏：
{
  ...
  // 或发足迹：
  "post": {"note": "50~150字第一人称足迹", "rating": 4, "spotIndex": 0, "photo": true, "photoDesc": "我用手机随手拍：窗边的手冲咖啡，目黑川的绿意在杯子后面虚化，最多一只手扶着杯子边缘"},  // rating 1-5 或 null；**大部分有场景/画面感的足迹都配图(约2/3)**；photoDesc 默认主观镜头。独自行动时不要写成别人拍发帖人的肖像，也不要让主角两只手同时清晰入镜，除非明确自拍/朋友帮拍/合照/倒影/背影
