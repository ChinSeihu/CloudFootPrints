...
- AI 导游以 `finish_reason` / `[DONE]` 校验 DeepSeek 流是否完整，并忽略空事件和未完成尾帧；首次请求在正文产生前中断或返回空正文时自动关闭思考重试一次。所有非流式思考任务共用正文校验与关闭思考重试；普通导游输出上限提高到 24000 token，高思考路线与长期人物决策提高到 32000 token，避免推理耗尽额度后没有最终正文。
