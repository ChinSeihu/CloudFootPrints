...
网页开屏固定使用用户选定的第一行第二款标志 `public/brand/splash-logo-02-wordmark.png`，不再按日期轮换。素材以 1024px 纯白底输出，并直接包含“云迹 / CLOUD FOOTPRINT”字标，确保 PWA 原生启动阶段也能显示；页面继续单独渲染副标题，PWA 与 Apple 启动图标使用同款缩放资源。
